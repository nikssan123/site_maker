...
- If hasDatabase: every rendered value that comes from fetched API/database records MUST be marked as dynamic/catalog-managed content for the platform editor. Add data-appmaker-dynamic="true" to the smallest practical JSX element that renders the value, and when known also add data-appmaker-model="<modelName>", data-appmaker-field="<fieldName>", and data-appmaker-id={String(record.id)}. Apply this to dynamic text, prices, descriptions, images, table cells, cards, detail pages, and list items. Do NOT add this marker to static UI labels, headings, buttons, navigation, or layout copy.
...
- A simple in-app translation dictionary/i18n setup is acceptable, but it must cover the full generated app.`;
  ...
  if (hasDatabase) {
    prompt += `\n\nDYNAMIC CONTENT MARKERS (REQUIRED): every rendered value that comes from API/database records must have data-appmaker-dynamic="true" on the smallest practical JSX element, plus data-appmaker-model, data-appmaker-field, and data-appmaker-id={String(record.id)} when known. Use these markers on fetched record text, images, prices, descriptions, table cells, list/card values, and detail-page values. Do not mark static labels, navigation, buttons, layout headings, or other non-record UI copy.`;
  }
...
5. For @mui/icons-material: stick to common, well-known icons (Home, Search, Menu, Person, Settings, Phone, Email, Star, Favorite, ShoppingCart, ArrowForward, ArrowBack, Close, Check, Add, Delete, Edit, LocationOn, AccessTime, CalendarToday, etc.). Avoid obscure icons that may not exist.`
      : '';
  return `Build/runtime error:\n\`\`\`\n${errorLog}\n\`\`\`${stepHint}\n\nCurrent source files:\n\n${fileList}\n\nPreserve any APPMAKER_LOGO_SLOT_START/END, APPMAKER_HERO_BG_START/END, and data-appmaker-dynamic markers that already exist; do not remove or rename them.`;
}
