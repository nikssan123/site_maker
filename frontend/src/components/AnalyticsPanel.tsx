              ...
              {data.daily.every((d) => d.views === 0) ? (
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <AreaChart data={data.daily} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
                    ...
                    />
                    <Area type="monotone" dataKey="views" stroke="#7c3aed" strokeWidth={2} fill="url(#adminPvGrad)" />
                  </AreaChart>
                </ResponsiveContainer>
              )}
            </Paper>
                    ...
                    <Stack spacing={1}>
                      {data.topPages.map((p) => (
                        <Stack key={p.path} direction="row" alignItems="center" justifyContent="space-between">
                          <Typography variant="body2" noWrap sx={{ maxWidth: 220, fontFamily: 'monospace', fontSize: 12 }}>
                            {p.path}
                          </Typography>
